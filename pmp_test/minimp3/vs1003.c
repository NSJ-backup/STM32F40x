...
static uint8_t VS1003_SPI_ReadWriteByte( uint8_t TxData )
{
	...
	if( HAL_SPI_TransmitReceive(&SpiHandle, &tx_data, &rx_data, 1, 0xFFFFFF) == HAL_OK)
	{
		return rx_data;
	}
	else
...
{		 
	...
	/* Configure PA.5(CLK),6(MISO),7(MOSI) pins: SPI1 */
	GPIO_InitStructure.Pin 			= SCLK_GPIO_PIN | MISO_GPIO_PIN | MOSI_GPIO_PIN;
	...
	HAL_GPIO_Init(VS1033B_GPIO, &GPIO_InitStructure);
	
	/* Configure PA.4(xCS),12(xDCS),10(Reset),8(DREQ) pins */
	GPIO_InitStructure.Pin 			= XCS_GPIO_PIN | XDCS_GPIO_PIN | RESET_GPIO_PIN ;
	GPIO_InitStructure.Mode 		= GPIO_MODE_OUTPUT_PP;		// push-pull output
	HAL_GPIO_Init(VS1033B_GPIO, &GPIO_InitStructure);
	
	GPIO_InitStructure.Pin 			= DREQ_GPIO_PIN ;
